...
|---|---|---|---|
| `ANTHROPIC_API_KEY` | `.env` (local), Render dashboard (deployed) | Yes | Authenticates calls to the Claude API for NL → SQL translation (used starting Day 4) |
...
| python-dotenv | 1.0.1 | Loads `ANTHROPIC_API_KEY` from `.env` into the environment |
| anthropic | 0.34.2 | Official SDK for calling the Claude API (used starting Day 4) |
...
| File | Purpose |
|---|---|
| `.env` | Holds `ANTHROPIC_API_KEY` locally. Gitignored. |
...
| `backend/schema.py` | Not a config file in the traditional sense, but functions as one: it's the single source of truth for what schema text gets sent to Claude in every AI prompt. Must stay in sync with `database/build_db.py` and `docs/SCHEMA.md`. |
